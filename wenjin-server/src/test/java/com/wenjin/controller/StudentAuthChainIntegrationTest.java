 ...
 * 用真实 HTTP 请求验证 {@code X-User-Id} 头 → CurrentUser → assertSelf 的端到端行为。
 *
 * <p>这是单测（mock 掉 CurrentUser）覆盖不到的层：它能抓住"请求没带身份头就放行"这类回归
 * （正是 SSE 原生 fetch 漏发 X-User-Id 导致 401 的同一类问题在后端侧的护栏）。
...
class StudentAuthChainIntegrationTest {
    ...
    @Test
    void getResult_noUserIdHeader_returns401() throws Exception {
        // 不带 X-User-Id：拦截器不填 CurrentUser → assertSelf 抛 UNAUTHORIZED
    ...
    @Test
    void submit_noUserIdHeader_returns401() throws Exception {
        // 写端点同样护住：无身份头不得替他人交卷
